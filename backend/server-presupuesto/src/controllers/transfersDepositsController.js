...
export const getAllTransactionsByAccount = async (req, res, next) => {
    ...
    let queryTransfersUniqueAccount = `SELECT "transfers"."transferId","transfers"."transferType","transfers".desctiption,"transfers".amount,"transfers"."shippingAccount","transfers"."destinationAccount","transfers"."userTransferId","transfers"."userReceivedId", "transfers"."createdAt", "userBudget"."userID","userBudget"."userName","userBudget"."userLastName","userBudget"."userEmail" FROM "transfers"
        ...
        let queryDepositsUniqueAccount = `SELECT "deposits"."depositId", "deposits"."depositType", "deposits"."desctiption", "deposits"."amount", "deposits"."shippingAccount", "deposits"."destinationAccount","deposits"."userTransferId","deposits"."userReceivedId", "deposits"."createdAt", "userBudget"."userID","userBudget"."userName","userBudget"."userLastName","userBudget"."userEmail" FROM "deposits"
        JOIN "userBudget" 
    ...
    try {
        ...
        if(resultTransfersAccount.rowCount === 0 && resultDepositsAccount.rowCount === 0) {
            return res.json({ msg: 'This account has not created any transactions yet.'})
        }
...
export const getAllTransactions = async (req, res, next) => {
    ...
    let queryTransfers = `SELECT "transfers"."transferId","transfers"."transferType","transfers".desctiption,"transfers".amount,"transfers"."shippingAccount","transfers"."destinationAccount","transfers"."userTransferId","transfers"."userReceivedId","userBudget"."userID","userBudget"."userName","userBudget"."userLastName","userBudget"."userEmail" FROM "transfers"
    JOIN "userBudget"
    ON "userBudget"."userID" = "transfers"."userReceivedId" WHERE "transfers"."userTransferId" = ${userID}`;

    // All the deposits I have recieved
    let queryDeposits = `SELECT "deposits"."depositId", "deposits"."depositType", "deposits"."desctiption", "deposits"."amount", "deposits"."shippingAccount", "deposits"."destinationAccount","deposits"."userTransferId","deposits"."userReceivedId", "userBudget"."userID","userBudget"."userName","userBudget"."userLastName","userBudget"."userEmail" FROM "deposits"
    JOIN "userBudget" 
    ...
    try {
        ...
        if(myTransfersResult.rowCount === 0 && myDepositsResult.rowCount === 0 ){
            return res.json({ msg: 'You do not have any transactions yet.'})
        }
...
export const findAccountReceiver = async (req, res, next) => {
    ...
    try {
        let query = `SELECT * FROM "userAccounts" where "accountNumber" = '${account}'`;
        const userAccount = await pool.query(query);

        if( userAccount.rowCount === 0){
            return res.json({ msg: `No users found with the account ${account}, check it and try again.`})
        }

        return res.status(200).json({ msg: 'User found', accountUser: userAccount.rows[0] });
...
export const newTransaction = async (req, res, next) => {
    ...
    const { userID } = req.user;
    const { transferType, description, amount, shippingAccount, destinationAccount, userReceivedId } = req.body;
    ...
    try {
        ...
        let queryTransfer = `INSERT INTO "transfers" ("transferType", "desctiption", "amount", "shippingAccount", "destinationAccount", "userTransferId", "userReceivedId") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`;
        let valuesTransfer = [transferType, description, amount, shippingAccount, destinationAccount, userID, userReceivedId];
        ...
        let queryDeposit = `INSERT INTO "deposits" ("depositType", "desctiption", "amount", "shippingAccount", "destinationAccount", "userTransferId", "userReceivedId") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`;
        ...
        return res.status(200).json({ msg: 'Transaction completed successfully.'})
